...
function finalizeResponseAfterBackfill(response: ApiResponse, limit: number, now: Date, timeMode: TrendTimeMode, previousCache: RankedTrend[]): ApiResponse {
  ...
  if (cleaned.length < limit || timeMode.mode === "early_morning_status") cleaned = [...cleaned, ...buildDailyRhythmRescueTrendsV2(cleaned, now)];
...
export function buildFinalTop10(validCandidates: RankedTrend[], previousCache: RankedTrend[] = [], timeMode: TrendTimeMode = getTrendTimeMode(), limit = 10, now: Date = new Date()): RankedTrend[] {
  ...
  if (pool.length < limit || timeMode.mode === "early_morning_status") pool = uniqueByTag([...pool, ...repairAndValidateMany(buildDailyRhythmRescueTrendsV2(pool, now), now)]);

  const liveSourceBacked = pool.filter((trend) => !isDailyRhythmTrend(trend));
  const baseDailyCap = liveSourceBacked.length >= timeMode.minLiveSourceBackedItems || timeMode.mode !== "early_morning_status"
    ? timeMode.dailyRhythmTop10Cap
    : Math.max(timeMode.dailyRhythmTop10Cap, 3);
  const dailyCap = timeMode.mode !== "early_morning_status" && nonDailyCount(pool) >= limit - 1 ? 1 : baseDailyCap;
...
function scoreForTimeMode(trend: RankedTrend, timeMode: TrendTimeMode): number {
  ...
  if (isDailyRhythmTrend(trend) && timeMode.mode !== "early_morning_status") score -= 18;
