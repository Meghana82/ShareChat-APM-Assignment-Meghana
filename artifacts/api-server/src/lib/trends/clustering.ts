...
function shouldMerge(cluster: TrendCluster, signal: FilteredSignal): boolean {
  ...
  return cluster.signals.some((existing) => {
    ...
    if (cricketSynonymMerge(existing.normalizedText, signal.normalizedText)) return true;
    return trainIncidentSynonymMerge(existing.normalizedText, signal.normalizedText);
  });
}
...
function trainIncidentSynonymMerge(a: string, b: string): boolean {
  const combined = `${a} ${b}`.toLowerCase();
  const trainTerms = ["ट्रेन", "train", "राजधानी", "एक्सप्रेस", "express", "रेल", "rail", "railway"].filter((term) => combined.includes(term)).length;
  const incidentTerms = ["आग", "fire", "हादसा", "accident", "दुर्घटना", "जलकर", "blast", "विस्फोट"].filter((term) => combined.includes(term)).length;
  return trainTerms >= 1 && incidentTerms >= 1;
}
